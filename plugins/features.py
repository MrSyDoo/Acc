...
retry_key = InlineKeyboardMarkup(
    [
        [
            InlineKeyboardButton(
                text="🔄 Retry",
                callback_data="retry"  # You can handle this callback in a callback query handler
            ),
            InlineKeyboardButton(
                text="❌ Cancel",
...
async def add_account(client, message):
    user_id = message.from_user.id
    Anony = client
    await message.reply("⏳ Trying to start Pyrogram v2 session generator...")
    ...
    try:
        ...
        return await client.send_message(user_id, "⏱ Timed out. Start again.", reply_markup=retry_key)
    if await cancelled(api_id_msg): return
    try:
        api_id = int(api_id_msg.text.strip())
    except ValueError:
        return await client.send_message(user_id, "❌ API ID must be an integer.", reply_markup=retry_key)
    ...
    try:
        ...
        return await Anony.send_message(user_id, "⏱ Timed out. Start again.", reply_markup=retry_key)
    if await cancelled(api_hash_msg): return
    api_hash = api_hash_msg.text.strip()
    if not api_hash or len(api_hash) < 30:
        return await Anony.send_message(user_id, "❌ Invalid API Hash.", reply_markup=retry_key)
    ...
    try:
        ...
        return await client.send_message(user_id, "⏱ Timed out. Start again.", reply_markup=retry_key)
    if await cancelled(phone_msg): return
    phone_number = phone_msg.text.strip()
    ...
    # Send code
    try:
        sent_code = await client2.send_code(phone_number)
    except FloodWait as f:
        return await client.send_message(user_id, f"⏱ Flood wait. Wait {f.value} seconds.", reply_markup=retry_key)
    except (ApiIdInvalid, PhoneNumberInvalid) as e:
        return await client.send_message(user_id, f"❌ Error: {e}", reply_markup=retry_key)
    ...
    try:
        ...
        return await client.send_message(user_id, "⏱ OTP timed out. Start again.", reply_markup=retry_key)
    ...
    try:
        ...
        if await cancelled(pwd_msg): return
        await client2.check_password(password=pwd_msg.text.strip())
    except (PhoneCodeInvalid, PhoneCodeExpired) as e:
        return await client.send_message(user_id, f"❌ Error: {e}", reply_markup=retry_key)

    # Export string session
    string_sess = await client2.export_session_string()
    await client.send_message(
        user_id,
        f"✅ Successfully generated your Pyrogram v2 string session:\n\n<code>{string_sess}</code>",
        reply_markup=InlineKeyboardMarkup([[InlineKeyboardButton("Saved Messages", url=f"tg://openmessage?user_id={user_id}")]]),
        disable_web_page_preview=True,
    )
    await client2.stop()
...
async def cancelled(message):
    ...
    if "/cancel" in text or text.startswith("/"):
        await message.reply("❌ Operation cancelled.", reply_markup=retry_key)
        return True
    return False
